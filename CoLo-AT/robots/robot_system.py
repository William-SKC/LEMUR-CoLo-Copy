...
from robot_centralized import CentralizedRobotSystem
from robot_distributive import DistributiveRobot
...
class RobotSystem:
	...
	def __init__(self, name, robot_labels, loc_algo, bound_algo = None, distr_sys = True,):
		...
		if distr_sys:
			...
			print('created a team of ', len(self.robot_sys), ' distibutive robots ')
	...
	def localization_update(self, rsp):
		...
		if rsp_type == 'odometry': #propogation update 
			...
			if message_data['subject_ID'] != None and message_data['subject_ID'] > 5: #landmark observation
				...
				if landmark_loc != None:
				    sensor_input = [landmark_loc, meas_range, bearing, message_data['time'], obj_id]
				    valid_update = True
				else: 
					valid_update = False
			else:
			...
			if self.distr_sys: 
				[comm_rbt_state, orientaions, comm_rbt_state_variance] = self.robot_sys[sender_idx].get_status()
			else:
				[comm_rbt_state, orientaions, comm_rbt_state_variance] = self.robot_sys.get_status()
		...
		if valid_update:
			if self.distr_sys:
				[est_states, est_orientaions, est_state_variance, update_type]= self.robot_sys[robot_index].state_update(update_type, sensor_data)
				trace_sigma_s = self.robot_sys[robot_index].get_trace_state_variance()
			else:
				[est_states, est_orientaions, est_state_variance, update_type]= self.robot_sys.state_update(robot_index, update_type, sensor_data)
				trace_sigma_s = self.robot_sys.get_trace_state_variance(robot_index)
		else: 
			if self.distr_sys:
				[est_states, est_orientaions, est_state_variance]= self.robot_sys[robot_index].get_status()
				trace_sigma_s = self.robot_sys[robot_index].get_trace_state_variance()
			else:
				[est_states, est_orientaions, est_state_variance]= self.robot_sys.get_status()
				trace_sigma_s = self.robot_sys.get_trace_state_variance(robot_index)

		robot_state = {'est_states':est_states, 'x_pos':est_states[2*robot_index], 'y_pos': est_states[2*robot_index+1], 'trace of state variance': trace_sigma_s, 'state variance': est_state_variance, 'update_type': update_type }
		return robot_state
