...
def plots_MSE_and_trace(dataset_labels, analysis_result_name, dataset_name, algo_name):
	...
	fig = plt.figure()
	plt.suptitle('Correctness analysis  Algo: ' + algo_name + ' Dataset: ' + dataset_name )
	...
	for i, label in enumerate(dataset_labels):
		...
		dev_l_RMSE = []
		dev_l_trace = []
		file="/Results_files/analysis_results/"+analysis_result_name+str(label)+".txt"
		path=os.getcwd()+file
...
def plots_mean_MSE_and_trace_for_multiple_algos(figure_name, algos_set, all_mean_time, all_mean_err, all_mean_STD, dataset_name, comm_fail_rate):
	...
	fig = plt.figure()
	plt.suptitle('Correctness analysis' + ' Dataset: ' + dataset_name + ' comm failure rate: ' + str(comm_fail_rate))
	fig1 = fig.add_subplot(211)
	fig2 = fig.add_subplot(212)
	...
	for i, algo_name in enumerate(algos_set):
		print algo_name
		if algo_name == 'GS-CI':
			fig1.plot(all_mean_time[i], all_mean_err[i], label = algo_name, linewidth=3.0)
			fig2.plot(all_mean_time[i], all_mean_STD[i], label = algo_name, linewidth=3.0)
		else: 
...
def plots_mean_MSE_and_trace_for_multiple_algos_w_bound(figure_name, algos_set, all_mean_time, all_mean_err, all_mean_STD, all_mean_bound, dataset_name, comm_fail_rate):
	...
	for i, algo_name in enumerate(algos_set):
		...
		if algo_name == 'GS-CI':
			fig1.plot(all_mean_time[i], all_mean_err[i], label = algo_name, linewidth=3.0)
			fig2.plot(all_mean_time[i], all_mean_STD[i], label = algo_name, linewidth=3.0)
			fig2.plot(all_mean_time[i], all_mean_bound[i], label = 'bound', linewidth=3.0)
		else: 
			fig1.plot(all_mean_time[i], all_mean_err[i], label = algo_name)
			fig2.plot(all_mean_time[i], all_mean_STD[i], label = algo_name)
...
def plot_RMSE_and_trace_withrespectto_noise(list2D_avg_RMSE, list2D_avg_trace, noise_range):
	...
	plt.figure(1)
	plt.suptitle('Error analysis for single EKF for all robots with respect to measurement noise')

	fig1  = plt.subplot(211)
	for i in range(5):
   		fig1.plot(list_process_noise, list2D_avg_RMSE[i], label= 'Robot %d' %(i+1))
	...
	for i in range(5):
   		fig2.plot(list_process_noise, list2D_avg_trace[i], label= 'Robot %d' %(i+1))	
...
def single_plot_w_bond(start_time, est_result_array, groundtruth_data, robot_index, algo_name):
	...
	while j < len(est_result_array[i]):
		...
		err_sq = sum([(est_x_pos-gt_x_pos)*(est_x_pos-gt_x_pos), (est_y_pos-gt_y_pos)*(est_y_pos-gt_y_pos)])
